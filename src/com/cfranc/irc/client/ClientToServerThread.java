...
public class ClientToServerThread extends Thread implements IfSenderModel{
	...
	// Entrée clavier , et flux d'entree /sortie pour la socket recue au constructor
	...
	// On recoit un message, quand pas précisé, ce sera par defaut gris pour le text et italique pour le user
	...
	public void receiveMessage(String user, String line, Style styleBI,
		...
		// A priori c'est plutot coté serveur qu'il faut garder tout le dialogue d'un salon
		// Et pouvoir renvoyer chaque message quand le user vient ou revient sur un salon
		// Coté client, il faudrait juste garder un ID, dernier message recu pour ce salon.
	...
	private boolean authentification() {
		...
		try {
			...
			if(acq.equals(IfClientServerProtocol.OK)){
				// Le serveur a accepté l'acquittement sera positif (return true)
				res=true;
			}
		} catch (IOException e) {
			// TODO Auto-generated catch block
